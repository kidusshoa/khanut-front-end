...
export const getBusinessInventory = async (
  businessId: string,
  options?: { search?: string; lowStock?: boolean }
) => {
...
export const updateProductStock = async (
  ...
  const response = await fetch(
    `${API_URL}/api/inventory/product/${productId}`,
    {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
...
export const batchUpdateStock = async (
  ...
  const response = await fetch(
    `${API_URL}/api/inventory/business/${businessId}/batch`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
export const getStockHistory = async (productId: string) => {
  ...
  const response = await fetch(
    `${API_URL}/api/inventory/product/${productId}/history`,
    {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
