...
export function OrderDetail({ order, customerId, onOrderUpdate }: OrderDetailProps) {
  ...
  const handlePayment = async () => {
    try {
      setIsLoading(true);
      const response = await paymentApi.initializeOrderPayment(order._id);
  ...
  const canCancel = ['pending_payment', 'payment_received', 'processing'].includes(order.status);
      ...
      <CardContent className="space-y-6">
            ...
            {order.items.map((item, index) => (
              <div key={index} className="flex justify-between items-center p-3 border rounded-md">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium">{item.serviceName || `Product #${item.serviceId}`}</p>
        ...
        <div>
          ...
          <Button 
            ...
            onClick={() => router.push(`/customer/${customerId}/businesses/${typeof order.businessId === 'string' ? order.businessId : order.businessId._id}`)}
          >
      ...
      <CardFooter className="flex flex-col sm:flex-row gap-3 pt-0">
        ...
        {order.status === 'shipped' && order.trackingNumber && (
          <Button 
            ...
            onClick={() => router.push(`/customer/${customerId}/orders/${order._id}/tracking`)}
          >
