...
import { CreditCard, Calendar, Clock, User, Building, FileText } from "lucide-react";
...
export default function TransactionDetailsModal({
  ...
  const handleViewRelatedItem = () => {
    if (transaction.paymentType === "order") {
      router.push(`/customer/${customerId}/orders/${transaction.referenceId}`);
    } else if (transaction.paymentType === "appointment") {
      router.push(`/customer/${customerId}/appointments/${transaction.referenceId}`);
    }
    onClose();
  };
          ...
          {transaction.businessId && (
            <div className="flex items-start gap-2">
              ...
              <div className="flex flex-col">
                ...
                <span>
                  {typeof transaction.businessId === "object" && transaction.businessId.name
                    ? transaction.businessId.name
                    : "Business ID: " + transaction.businessId}
                </span>
              </div>
            </div>
          )}
