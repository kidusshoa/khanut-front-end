...
export default function CategoryDetailPage({
  ...
  useEffect(() => {
    const fetchCategoryDetails = async () => {
      try {
        ...
        const servicesData = await categoryApi.getServicesByCategory(categoryId);
  ...
  useEffect(() => {
    ...
    if (serviceType !== "all") {
      filtered = filtered.filter((service) => service.serviceType === serviceType);
    }
    ...
    <div className="container mx-auto py-8 px-4">
      ...
      {filteredServices.length > 0 ? (
        ...
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          {serviceType !== "all" && (
            <Button onClick={() => setServiceType("all")} className="bg-orange-600 hover:bg-orange-700">
              Show All Services
            </Button>
          )}
        </div>
