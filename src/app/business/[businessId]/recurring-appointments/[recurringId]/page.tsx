...
export default function RecurringAppointmentDetailsPage({
  ...
  const [recurringAppointment, setRecurringAppointment] = useState<RecurringAppointment | null>(null);
  ...
  const fetchRecurringAppointmentDetails = async () => {
    try {
      setIsLoading(true);
      const response = await recurringAppointmentApi.getRecurringAppointmentById(recurringId);
  ...
  const handleUpdateStatus = async (status: "active" | "paused" | "completed" | "cancelled") => {
    try {
  ...
  const handleDeleteRecurringAppointment = async () => {
    try {
      await recurringAppointmentApi.deleteRecurringAppointment(recurringId, deleteFutureOnly);
      toast({
        title: "Success",
      ...
      <div className="container mx-auto py-8 px-4">
        <div className="mb-6">
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push(`/business/${businessId}/recurring-appointments`)}
            className="mb-4"
          >
        ...
        {recurringAppointment ? (
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    className={cn(
                      "ml-2",
                      recurringAppointment.status === "active" && "bg-green-500",
                      recurringAppointment.status === "paused" && "bg-yellow-500",
                      recurringAppointment.status === "completed" && "bg-blue-500",
                      recurringAppointment.status === "cancelled" && "bg-red-500"
                    )}
                  >
                    {recurringAppointment.status.charAt(0).toUpperCase() + recurringAppointment.status.slice(1)}
                  </Badge>
                </div>
                <p className="text-muted-foreground">
                  Created on {format(parseISO(recurringAppointment.createdAt), "MMMM d, yyyy")}
                </p>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {(recurringAppointment.status === "active" || recurringAppointment.status === "paused") && (
                  <Button
                    variant="outline"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    <h3 className="text-sm font-medium text-muted-foreground mb-1">Recurrence Pattern</h3>
                    <p>
                      {formatRecurrencePattern(
                        recurringAppointment.recurrencePattern,
                        recurringAppointment.dayOfWeek,
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span>
                        From: {format(parseISO(recurringAppointment.startDate), "MMMM d, yyyy")}
                      </span>
                    </div>
                    {recurringAppointment.endDate && (
                      <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-muted-foreground" />
                        <span>
                          To: {format(parseISO(recurringAppointment.endDate), "MMMM d, yyyy")}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        {recurringAppointment.startTime} - {recurringAppointment.endTime}
                      </span>
                    </div>
                  </div>
          ...
          <div className="flex flex-col items-center justify-center py-12">
            <Info className="h-12 w-12 text-muted-foreground mb-4" />
            <h3 className="text-lg font-medium mb-2">Recurring Appointment Not Found</h3>
            <p className="text-muted-foreground text-center max-w-md mb-4">
              The recurring appointment you're looking for could not be found.
            </p>
            <Button onClick={() => router.push(`/business/${businessId}/recurring-appointments`)}>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            ...
            <div className="bg-yellow-50 dark:bg-yellow-950/30 p-3 rounded-md border border-yellow-200 dark:border-yellow-900">
              <div className="flex gap-2">
                ...
                <div>
                  <p className="text-sm text-yellow-800 dark:text-yellow-400 font-medium">Warning</p>
                  <p className="text-xs text-yellow-700 dark:text-yellow-500 mt-1">
                    This action cannot be undone. Deleting a recurring appointment will remove it from your schedule.
                  </p>
                </div>
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button variant="destructive" onClick={handleDeleteRecurringAppointment}>
              Delete
            </Button>
          </DialogFooter>
        </DialogContent>
