...
const fetchBusinessServices = async (businessId: string) => {
  try {
    ...
    if (!response.ok) {
      console.error("Services endpoint failed:", response.status, response.statusText);
      throw new Error(`Failed to fetch business services: ${response.status} ${response.statusText}`);
    }
...
export default function BusinessServicesPublicPage({
    ...
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="flex flex-col items-center gap-2 max-w-md text-center">
          ...
          <p className="text-muted-foreground">
            The business you're looking for could not be found or may have been removed.
          </p>
          <Button onClick={() => router.push("/")} className="mt-4">
            Return to Home
          </Button>
  ...
  const appointmentCount = services.filter((s: Service) => s.serviceType === "appointment").length;
  const productCount = services.filter((s: Service) => s.serviceType === "product").length;
  const inPersonCount = services.filter((s: Service) => s.serviceType === "in_person").length;
                ...
                <Card key={service._id} className="overflow-hidden flex flex-col">
                    ...
                    <div className="space-y-2">
                      ...
                      {service.serviceType === "product" && service.inventory !== undefined && (
                        <div className="flex justify-between">
                          ...
                          <span>{service.inventory > 0 ? `${service.inventory} available` : "Out of stock"}</span>
                        </div>
                      )}
                    </div>
                  </CardContent>
                  ...
                  <CardFooter className="border-t pt-4">
                    {service.serviceType === "appointment" ? (
                      <Button 
                        className="w-full" 
                        onClick={() => router.push(`/business/${businessId}/service/${service._id}/book`)}
                      >
                      ...
                      <Button 
                        ...
                        onClick={() => router.push(`/business/${businessId}/service/${service._id}/purchase`)}
                      >
                      ...
                      <Button 
                        className="w-full" 
                        variant="outline"
                        onClick={() => router.push(`/business/${businessId}/service/${service._id}/details`)}
                      >
